...
class ProductosModelo
{
    ...
    static public function mdlCargaMasivaProductos($fileProductos)
    {
        ...
        for ($i = 2; $i <= $numeroFilasCategorias; $i++) {
            ...
            $categoria = $hojaCategorias->getCell('A' . $i)->getValue();
            $aplica_peso = $hojaCategorias->getCell('B' . $i)->getValue();
        ...
        if ($categoriasRegistradas > 0) {
            ...
            for ($i = 2; $i <= $numeroFilasProductos; $i++) {
                ...
                $marca_producto = $hojaProductos->getCell("D" . $i);
                $ubicacion_producto = $hojaProductos->getCell("E" . $i);
                $fecha_vencimiento_producto = $hojaProductos->getCell("F" . $i);
                $precio_compra_producto = $hojaProductos->getCell("G" . $i);
                $precio_venta_producto = $hojaProductos->getCell("H" . $i)->getCalculatedValue();
                $precio_mayor_producto = $hojaProductos->getCell("I" . $i)->getCalculatedValue();
                $precio_oferta_producto = $hojaProductos->getCell("J" . $i)->getCalculatedValue();
                $utilidad = $hojaProductos->getCell("K" . $i)->getCalculatedValue();
                $impuesto_producto_iva = $hojaProductos->getCell("L" . $i);
                $stock_producto = $hojaProductos->getCell("M" . $i);
                $minimo_stock_producto = $hojaProductos->getCell("N" . $i);
                $ventas_producto = $hojaProductos->getCell("O" . $i);
                ...
                if (!empty($codigo_producto) && strlen($codigo_producto) > 0) {
                    ...
                    $stmt->bindParam(":ubicacion_producto", $ubicacion_producto, PDO::PARAM_STR);
                    $stmt->bindParam(":fecha_vencimiento_producto", $fecha_vencimiento_producto, PDO::PARAM_STR);
                    ...
                    $stmt->bindParam(":impuesto_producto_iva", $impuesto_producto_iva, PDO::PARAM_STR);
    ...
    static public function mdlGetDatosProducto($codigoProducto)
    {
        try {
            ...
            // Manejo de errores en caso de una excepción PDO
            // Aquí puedes registrar el error, lanzar una excepción personalizada o tomar alguna otra acción.
